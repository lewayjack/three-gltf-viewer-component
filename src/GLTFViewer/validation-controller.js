...
export class ValidationController {
  ...
  validate(rootFile, rootPath, assetMap, response) {
    ...
    return fetch(rootFile)
      ...
      .then((report) => {
        this.setReport(report, response);
  ...
  setReport(report, response) {
    ...
    const generator = registry.generators.find((tool) => {
  ...
  showLightbox() {
    ...
    tab.document.body.innerHTML = this.reportTpl(
      Object.assign({}, this.report, { location: window.location })
    );
  }
}
